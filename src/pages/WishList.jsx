...
export const WishList = () => {
  ...
  useEffect(() => {
    ...
    if (storedWishList) {
      //index === storedWishList.findIndex(...) 으로 중복된 항목은 제외하기
      const uniqueMovies = storedWishList.filter(
        (movie, index) =>
          index === storedWishList.findIndex((m) => m.id === movie.id)
      );
  ...
  const handleDelete = (movieId) => {
    // 해당 영화하는 영화만 고르기
    const updateMovies = wishList.filter((movie) => movie.id !== movieId);
    //로컬스토리지에 업데이트된 영화 목록으로 저장하기
    localStorage.setItem("wishList", JSON.stringify(updateMovies));
    // 상태를 업데이트
    setWishList(updateMovies);
  };
