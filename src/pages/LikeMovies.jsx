...
export const LikeMovies = () => {
  ...
  useEffect(() => {
    const storedLikeMovies = JSON.parse(localStorage.getItem("likeMovies"));
    if (storedLikeMovies) {
  ...
  const handleDelete = (movieId) => {
    // 해당 영화하는 영화만 고르기
    const updateMovies = likeMovies.filter((movie) => movie.id !== movieId);
    //로컬스토리지에 업데이트된 영화 목록으로 저장하기
    localStorage.setItem("likeMovies", JSON.stringify(updateMovies));
    // 상태를 업데이트
    setLikeMovies(updateMovies);
  };
